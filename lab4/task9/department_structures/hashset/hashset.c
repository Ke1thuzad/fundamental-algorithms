...
int insert_node(HashTable *table, Department *department) {
    int res =  insert_node_hash(table, department, table->size);
...
int insert_node_hash(HashTable *table, Department *department, unsigned long hash) {
    ...
    err = create_node(node, department);
    if (err)
        return err;
    ...
    for (int i = 0; i < ht->size; i++) {
        ...
        while (current != NULL) {
            if (new_ht->size >= current->hash)
                insert_node_hash(new_ht, current->department, new_ht->size);
            else
                insert_node_hash(new_ht, current->department, current->hash);
